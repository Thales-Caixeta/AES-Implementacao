package aes;

import java.util.Arrays;

/**
 * AES puro (sem libs) — suporta chaves de 128/192/256 bits.
 * Implementa: SubBytes, ShiftRows, MixColumns, AddRoundKey, KeyExpansion
 * e as versões inversas para decriptação.
 */
public class AES {
    private static final int Nb = 4; // colunas do estado

    // S-Box e inversa (tabelas oficiais do AES)
    private static final int[] SBOX = new int[] {
        
    };
    private static final int[] INV_SBOX = new int[] {
        
    };

    // Rcon oficial (suficiente para até 14 rounds — AES-256)
    private static final int[] RCON = new int[] {
        0x00000000,
        0x01000000,0x02000000,0x04000000,0x08000000,0x10000000,0x20000000,0x40000000,0x80000000,0x1b000000,0x36000000,
        0x6c000000,0xd8000000,0xab000000,0x4d000000,0x9a000000
    };

    private final int Nk; // palavras na chave (4,6,8)
    private final int Nr; // rounds (10,12,14)
    private final int[] roundKeys; // Nb*(Nr+1) palavras

    public AES(byte[] key) {
        if (key == null || !(key.length == 16 || key.length == 24 || key.length == 32))
            throw new IllegalArgumentException("Chave deve ter 16, 24 ou 32 bytes");
        this.Nk = key.length / 4;
        this.Nr = Nk + 6;
        this.roundKeys = keyExpansion(key);
    }

    // ======= API: cifra/decifra um bloco (16 bytes) =======
    public byte[] encryptBlock(byte[] in) {
        if (in.length != 16) throw new IllegalArgumentException("Bloco deve ter 16 bytes");
        byte[] s = Arrays.copyOf(in, 16);

        addRoundKey(s, 0);
        for (int r = 1; r < Nr; r++) {
            subBytes(s);
            shiftRows(s);
            mixColumns(s);
            addRoundKey(s, r);
        }
        subBytes(s);
        shiftRows(s);
        addRoundKey(s, Nr);
        return s;
    }

    public byte[] decryptBlock(byte[] in) {
        if (in.length != 16) throw new IllegalArgumentException("Bloco deve ter 16 bytes");
        byte[] s = Arrays.copyOf(in, 16);

        addRoundKey(s, Nr);
        for (int r = Nr - 1; r >= 1; r--) {
            invShiftRows(s);
            invSubBytes(s);
            addRoundKey(s, r);
            invMixColumns(s);
        }
        invShiftRows(s);
        invSubBytes(s);
        addRoundKey(s, 0);
        return s;
    }

    // ======= Operações de round =======
    private static void subBytes(byte[] s) {
        for (int i = 0; i < 16; i++) s[i] = (byte) SBOX[s[i] & 0xff];
    }
    private static void invSubBytes(byte[] s) {
        for (int i = 0; i < 16; i++) s[i] = (byte) INV_SBOX[s[i] & 0xff];
    }

    // estado armazenado por colunas: idx = r + 4*c
    private static void shiftRows(byte[] s) {
        byte t;
        t = s[1]; s[1]=s[5]; s[5]=s[9]; s[9]=s[13]; s[13]=t;
        t = s[2]; s[2]=s[10]; s[10]=t; t=s[6]; s[6]=s[14]; s[14]=t;
        t = s[3]; s[3]=s[15]; s[15]=s[11]; s[11]=s[7]; s[7]=t;
    }
    private static void invShiftRows(byte[] s) {
        byte t;
        t = s[13]; s[13]=s[9]; s[9]=s[5]; s[5]=s[1]; s[1]=t;
        t = s[2];  s[2]=s[10]; s[10]=t; t=s[6]; s[6]=s[14]; s[14]=t;
        t = s[7];  s[7]=s[11]; s[11]=s[15]; s[15]=s[3]; s[3]=t;
    }

    private static void mixColumns(byte[] s) {
        for (int c = 0; c < 4; c++) {
            int i = 4*c;
            byte a0 = s[i], a1 = s[i+1], a2 = s[i+2], a3 = s[i+3];
            s[i]   = (byte)(gmul(a0,2) ^ gmul(a1,3) ^ a2 ^ a3);
            s[i+1] = (byte)(a0 ^ gmul(a1,2) ^ gmul(a2,3) ^ a3);
            s[i+2] = (byte)(a0 ^ a1 ^ gmul(a2,2) ^ gmul(a3,3));
            s[i+3] = (byte)(gmul(a0,3) ^ a1 ^ a2 ^ gmul(a3,2));
        }
    }
    private static void invMixColumns(byte[] s) {
        for (int c = 0; c < 4; c++) {
            int i = 4*c;
            byte a0 = s[i], a1 = s[i+1], a2 = s[i+2], a3 = s[i+3];
            s[i]   = (byte)(gmul(a0,0x0e) ^ gmul(a1,0x0b) ^ gmul(a2,0x0d) ^ gmul(a3,0x09));
            s[i+1] = (byte)(gmul(a0,0x09) ^ gmul(a1,0x0e) ^ gmul(a2,0x0b) ^ gmul(a3,0x0d));
            s[i+2] = (byte)(gmul(a0,0x0d) ^ gmul(a1,0x09) ^ gmul(a2,0x0e) ^ gmul(a3,0x0b));
            s[i+3] = (byte)(gmul(a0,0x0b) ^ gmul(a1,0x0d) ^ gmul(a2,0x09) ^ gmul(a3,0x0e));
        }
    }

    private void addRoundKey(byte[] s, int round) {
        for (int c = 0; c < Nb; c++) {
            int w = roundKeys[round*Nb + c];
            s[4*c]   ^= (byte)((w >>> 24) & 0xff);
            s[4*c+1] ^= (byte)((w >>> 16) & 0xff);
            s[4*c+2] ^= (byte)((w >>> 8)  & 0xff);
            s[4*c+3] ^= (byte)( w         & 0xff);
        }
    }

    // ======= Key schedule =======
    private int[] keyExpansion(byte[] key) {
        int words = Nb * (Nr + 1);
        int[] w = new int[words];

        // copia chave inicial
        for (int i = 0; i < Nk; i++) {
            w[i] = ((key[4*i] & 0xff) << 24) | ((key[4*i+1] & 0xff) << 16)
                 | ((key[4*i+2] & 0xff) << 8) | (key[4*i+3] & 0xff);
        }

        for (int i = Nk; i < words; i++) {
            int temp = w[i - 1];
            if (i % Nk == 0) {
                temp = subWord(rotWord(temp)) ^ RCON[i / Nk];
            } else if (Nk > 6 && i % Nk == 4) {
                temp = subWord(temp);
            }
            w[i] = w[i - Nk] ^ temp;
        }
        return w;
    }

    private static int rotWord(int x) { return (x << 8) | ((x >>> 24) & 0xff); }
    private static int subWord(int x) {
        return (SBOX[(x >>> 24) & 0xff] << 24)
             | (SBOX[(x >>> 16) & 0xff] << 16)
             | (SBOX[(x >>> 8)  & 0xff] << 8)
             |  SBOX[x & 0xff];
    }

    // multiplicação em GF(2^8) com polinômio redutor 0x11B
    private static byte gmul(byte a, int b) {
        int aa = a & 0xff, bb = b & 0xff, res = 0;
        for (int i = 0; i < 8; i++) {
            if ((bb & 1) != 0) res ^= aa;
            boolean hi = (aa & 0x80) != 0;
            aa = (aa << 1) & 0xff;
            if (hi) aa ^= 0x1b;
            bb >>>= 1;
        }
        return (byte) res;
    }

    // getters úteis no selftest
    public int getNr() { return Nr; }
    public int getNk() { return Nk; }
}
